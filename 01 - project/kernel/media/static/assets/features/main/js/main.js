...
$(document).ready(function () {
  ...
  var x = setInterval(function () {
    ...
    var days = Math.floor(distance / (1000 * 60 * 60 * 24 * 150));
    var hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
    var minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
    var seconds = Math.floor((distance % (1000 * 60)) / 1000);

    // Display the result in the element with id="firstClock"
    document.getElementById("firstClock").innerHTML = seconds + " : " + minutes + " : " + hours + " : " + days + "";

    // If the count down is finished, write some text 
    if (distance < 0) {
      clearInterval(x);
      document.getElementById("firstClock").innerHTML = "EXPIRED";
    }
  }, 1000);
});
